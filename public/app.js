...
function parseReportViolations(html) {
  ...
  // Try our tool's format first (.rule-section), then IBM format (.rule-group)
  const sections = doc.querySelectorAll('.rule-section');
  if (sections.length) return parseOwnReportViolations(doc);

  const ruleGroups = doc.querySelectorAll('.rule-group');
  if (ruleGroups.length) return parseIBMReportViolations(doc);

  return [];
}

// Parse reports generated by this tool (axe-core format)
function parseOwnReportViolations(doc) {
  const sections = doc.querySelectorAll('.rule-section');
  const violations = [];
  ...
  for (const section of sections) {
    ...
    violations.push({
      ruleId, impact, wcagCriteria, wcagTagText,
...
// Parse IBM Equal Access Checker / Acadea violation reports
// Structure: .catalog-group > .rule-group > .url-group > .violation-details
function parseIBMReportViolations(doc) {
  const ruleGroups = doc.querySelectorAll('.rule-group');
  const violations = [];
  ...
  for (const group of ruleGroups) {
    ...
    // Extract rule ID from "Rule: rule_name" text
    const summaryText = summary.textContent.trim();
    const ruleMatch = summaryText.match(/^Rule:\s*(\S+)/i);
    const ruleId = ruleMatch ? ruleMatch[1] : summaryText;

    // Look up WCAG criteria via mapping tables
    const wcagCriteria = [];
    const mapped = IBM_TO_WCAG[ruleId] || AXE_TO_WCAG[ruleId];
    if (mapped) wcagCriteria.push(mapped);

    // Collect all URLs from nested .url-group elements
    const urlGroups = group.querySelectorAll('.url-group');
    const urls = new Set();
    let instanceCount = 0;

    for (const urlGroup of urlGroups) {
      const linkEl = urlGroup.querySelector(':scope > summary a[href]');
      if (linkEl) {
        const href = linkEl.getAttribute('href') || linkEl.textContent.trim();
        urls.add(normalizeUrl(href));
      }
      // Count violation-details as instances
      instanceCount += urlGroup.querySelectorAll('.violation-details').length;
    }
...
function handleBaselineUpload(file) {
  ...
  reader.onload = function(e) {
    ...
    try {
      ...
      const hasOwnRules = testDoc.querySelectorAll('.rule-section').length > 0;
      const hasIBMRules = testDoc.querySelectorAll('.rule-group').length > 0;
      const hasHeader = testDoc.querySelector('.report-header');
      const hasAllClear = testDoc.querySelector('.all-clear');

      if (!hasOwnRules && !hasIBMRules && !hasHeader && !hasAllClear) {
